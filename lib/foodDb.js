...
const RAW = [
  ...
  ['Farine de blé', 'Céréales & féculents', 'gluten:h fructanes:m', 'farine'],
...
// Is the key phrase `kw` present as a CONTIGUOUS run of words inside `qw`?
// Contiguity matters when parsing a full recipe: without it, "huile" from one
// step and "olive" from another would falsely match "Huile d'olive". Requiring
// adjacency keeps extraction precise on long, noisy OCR text.
function phraseInWords(kw, qw) {
  if (!kw.length || kw.length > qw.length) return false;
  for (let i = 0; i + kw.length <= qw.length; i++) {
    let ok = true;
    for (let j = 0; j < kw.length; j++) {
      if (qw[i + j] !== kw[j]) { ok = false; break; }
    }
    if (ok) return true;
  }
  return false;
...
// Extract every food mentioned in a free-text block (speech transcript OR the
// full text of a recipe read by OCR). A food matches when one of its key phrases
// (name or alias, singularised) appears as a contiguous run of words. Longer
// phrases win over their parts (e.g. "Huile d'olive" over "Huile").
function extract(text) {
  ...
  FOODS.forEach(function (f) {
    let best = 0;
    f._keys.forEach(function (kw) {
      if (kw.length && kw.length > best && phraseInWords(kw, qw)) best = kw.length;
    });
    if (best && !seen[f.name]) {
      seen[f.name] = 1;
